...
def risk_summation(model):
    return sum((model.alpha[i]) * (model.p[i] * model.p[i]) for i in model.i)
...
def rash(rsc_budget, tasks, time_slice):
    print("resource budget", f'{rsc_budget:,}')
    backhaul_bandwidth_budget = 6 * (10**4) # 6KB
    ...
    model.Constraint10 = pyo.Constraint(model.i, rule=rule_privacy_leakage)
    model.Constraint11 = pyo.Constraint(model.i, rule=risk_auxiliary)

    # objective function
    model.obj_primary = pyo.Objective(expr=obj_expression(model), sense=pyo.minimize)
    ...
    try:
        ...
        model.obj_primary.deactivate()
        if results.solver.termination_condition == TerminationCondition.optimal:
            optimal_R = pyo.value(model.R)
            print("first objective was feasible")
        else:
            print("Problem is infeasible or not optimal. Cannot access R.")
            return model, results.solver.termination_condition

        # instance.display()
        # Add a constraint to fix R
        model.ConstraintR = pyo.Constraint(expr=model.R == optimal_R)

        # Redefine the objective to maximize CPU utilization
        model.OBJ = pyo.Objective(rule=risk_summation, sense=pyo.minimize)
