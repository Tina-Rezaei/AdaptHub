...
module.exports = class SpeechRecognition {
    ...
    async run(cameraDevice, eventEngine) {
        ...
        logger.log.info("Butler: started speech recognition");
        logger.timeLog("butler-app-fetching-audio", "start");
        let pcm = await VoiceAssistant.getLastVoiceCommandAudio();
        logger.timeLog("butler-app-fetching-audio", "end");
        // console.log(pcm);
        logger.timeLog("butler-app-speech-recognition","start") 
...
//         console.log("here is hypothesis in local app");
//         console.log(hypothesis);
        // Again we must manually release memory
        // this.decoder.delete();
        logger.timeLog("butler-app-speech-recognition","end") 
